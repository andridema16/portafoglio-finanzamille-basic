...
import type { UserRole, PortfolioId } from "@/types/portafoglio";
import { getNavItems, getPortfolioMeta } from "@/lib/portfolio";
...
export default function Sidebar({ ruolo, portfolioId }: Props) {
  const pathname = usePathname();
  const [mobileOpen, setMobileOpen] = useState(false);
  const [gestioneOpen, setGestioneOpen] = useState(true);

  const navItems = getNavItems(portfolioId);
  const portfolioMeta = getPortfolioMeta(portfolioId);
  ...
  const navContent = (
    <nav className="flex flex-col h-full">
      ...
      <div className="px-5 py-6 border-b border-white/10">
        <Link href={`/${portfolioId}/dashboard`} className="flex items-center gap-3">
          <Image src="/icon.png" alt="FinanzaMille" width={32} height={32} />
          <span className="text-white font-semibold text-sm">{portfolioMeta.nome}</span>
        </Link>
      </div>
      ...
      <div className="px-3 py-4 border-t border-white/10 space-y-1">
        <div className="px-3 py-2">
          ...
          <p className="text-white text-sm font-semibold">
            {portfolioMeta.nome.replace("Portafoglio ", "")}
          </p>
        </div>
        ...
        <Link
          href="/watchlist"
          onClick={() => setMobileOpen(false)}
          className="flex items-center justify-between px-3 py-2.5 rounded-lg text-sm text-white/70 hover:bg-white/10 hover:text-white transition-colors"
        >
          <span>Watchlist</span>
        ...
        <Link
          ...
          onClick={() => setMobileOpen(false)}
          className="block px-3 py-1 text-white/40 text-xs hover:text-white/70 transition-colors"
        >
          Tutti i portafogli
        </Link>
